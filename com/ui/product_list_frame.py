...
from com.dao.product_dao import ProductDao
from com.ui.my_frame import MyFrame
from com.ui.cart_frame import CartFrame
...
class ProductListFrame(MyFrame):
    ...
    def createrightpanel(self, parent):
        ...
        # 显示第一张照片
        image_path = 'resources/images/' + self.data[0]['image']
        image = wx.Bitmap(image_path, wx.BITMAP_TYPE_ANY)  # 第二个参数设置图片格式可以是任意的
        image_static_bitmap = wx.StaticBitmap(panel, bitmap=image, name='image_static_bitmap')

        # 商品市场价
        list_price = "商品市场价：￥{0:.2f}".format(self.data[0]['listprice'])
        list_price_st = wx.StaticText(panel, label=list_price, name='listprice')
        # 商品单价
        ...
        box.AddSpacer(30)
        box.Add(image_static_bitmap, 1, flag=wx.CENTER | wx.ALL, border=30)
        box.AddSpacer(40)
        box.Add(list_price_st, 1, flag=wx.EXPAND | wx.ALL, border=10)
    ...
    def selectrow_handler(self, event):
        ...
        if srowidx >= 0:
            ...
            # 显示选择的图片
            image_path = 'resources/images/' + self.selecteddata['image']
            image = wx.Bitmap(image_path, wx.BITMAP_TYPE_ANY)
            # 通过名称查询子窗口
            image_sbitmap = self.FindWindowByName('image_static_bitmap')
            ...
            # 商品描述
            sdescn = "商品描述：{0}".format(self.selecteddata['descn'])
            descn_st = self.FindWindowByName('descn')
            descn_st.SetLabelText(sdescn)
