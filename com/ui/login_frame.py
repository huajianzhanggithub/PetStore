...
class LoginFrame(MyFrame):
    def __init__(self):
        ...
        self.password_txt = wx.TextCtrl(self.contentpanel, style=wx.TE_PASSWORD | wx.TE_PROCESS_ENTER)
        ...
        # 监听密码文本框回车事件
        self.Bind(wx.EVT_TEXT_ENTER, self.okb_btn_onclick, self.password_txt)

        # 创建水平Box布局hbox对象
        hbox = wx.BoxSizer(wx.HORIZONTAL)
        hbox.AddMany([(okb_btn, 1, wx.CENTER | wx.ALL | wx.EXPAND, 10),
                      (cancel_btn, 1, wx.CENTER | wx.ALL | wx.EXPAND, 10)])

        # 创建垂直Box布局,把fgs和hbox添加到垂直Box布局对象上
        vbox = wx.BoxSizer(wx.VERTICAL)
        vbox.AddMany([(fgs, -1, wx.CENTER | wx.ALL | wx.EXPAND, 25),
                      (hbox, -1, wx.CENTER | wx.BOTTOM, 20)])
